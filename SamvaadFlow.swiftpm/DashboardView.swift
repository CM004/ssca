...
struct DashboardView: View {
    ...
    private var startTokens: Int { appState.tokenHistory.first ?? domainConfig.startingTokens }
    ...
    private func estimateExchanges() async {
        if #available(iOS 26, *) {
            do {
                ...
                let prompt = """
                A user sends this vague prompt to an AI: "\(domainConfig.startingPrompt)"
    ...
    private func loadInfo(for card: InfoCard) async {
        ...
        if #available(iOS 26, *) {
            do {
                ...
                Starting prompt: "\(domainConfig.startingPrompt)" (\(startTokens) tokens)
